...
const Navbar = ({ nameVisible }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [showNameInNavbar, setShowNameInNavbar] = useState(false);

  useEffect(() => {
    if (nameVisible) {
      setTimeout(() => {
        setShowNameInNavbar(true); // Show name in navbar after 1 second
      }, 1000);
    }
  }, [nameVisible]);
          ...
          <div className="absolute right-0 flex items-center">
            <div className="hidden sm:block sm:ml-6">
              <div className="flex space-x-4">
                <a href="#HeroSection" className="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">Home</a>
